...
import tomolab as __occiput
from tomolab.Core.Errors import *
from tomolab.Core.NiftyRec import  PET_compress_projection, \
...
def display_PET_Projection_geometry():
    ...
    img = Image(filename = filename,width = 800)
    return img
...
DEFAULT_BINNING = {
    "n_axial":120,
    "n_azimuthal":5,
    "angles_axial":np.float32(np.linspace(0,np.pi - np.pi / 120.0,120)),
    "angles_azimuthal":np.float32([-0.5,-0.25,0.0,0.25,0.5]),
    "size_u":256.0,
    "size_v":256.00,
    "n_u":128,
    "n_v":128,
...
class Binning():
    ...
    def load_from_dictionary(self,dictionary):
        '''
        ["n_axial"]  # Number of axial bins
        ["n_azimuthal"]  # Number of azimuthal bins
        ["angles_axial"]  # Axial angles radians
        ["angles_azimuthal"]  # Azimuthal angles radians
        ["size_u"]  # Size of thedetector plane, axis u, [adimensional]
        ["size_v"]  # Size of the detector plane, axis v,  [adimensional]
    ...
    def get_angles(self,subsets_matrix = None):
        angles = np.zeros((2,self.N_azimuthal,self.N_axial))
        angles[0,:,:] = np.tile(self.angles_axial,(self.N_azimuthal,1))
        angles[1,:,:] = np.tile(self.angles_azimuthal,(self.N_axial,1)).T

        if subsets_matrix is not None:
            subsets_matrix = np.int32(subsets_matrix)
            N = subsets_matrix.sum()
            angles2 = angles.reshape(2,self.N_azimuthal * self.N_axial).copy()
            angles = np.zeros([2,1,N])
            angles[0,0,:] = (angles2[0,subsets_matrix.flatten() == 1]).reshape(
                    [N])
            angles[1,0,:] = (angles2[1,subsets_matrix.flatten() == 1]).reshape(
                    [N])
        return np.float32(angles)
    ...
    def _repr_html_(self):
        ...
        if i_end - i_start >= 4:
            table_data.append(["Angles_axial [deg]",
...
class PET_Projection_Sparsity:
    ...
    def __init__(self,N_axial,N_azimuthal,N_u,N_v,offsets = None, locations = None):
        ...
        self.N_axial = np.int32(N_axial)
        self.N_azimuthal = np.int32(N_azimuthal)
        self.N_u = N_u
        self.N_v = N_v
    ...
    def get_N_locations(self):
        ...
        return np.int32(self.locations.shape[1])
    ...
    def get_subset(self, subset_matrix):
        if subset_matrix is None:
            return self
        subset_matrix = np.int32(subset_matrix)
        N_locations = self.get_N_locations()
        N_subsets = subset_matrix.sum()
        N_locations_sub = np.int32(
    ...
    def __get_subset_pure_python_version_deprecated(self,subset_matrix):
        ...
        offsets_return = np.zeros((1,N), dtype= np.int32, order="F")
        sizes = self.get_locations_per_plane().reshape(indexes.shape)

        N_locations = sizes[indexes].sum()
        locations_return = np.zeros((3,N_locations), dtype=np.uint16, order="F")
...
class PET_Projection:
    ...
    def __init__(
        ...
        N_axial = np.int32(self.angles.shape[2])
        N_azimuthal = np.int32(self.angles.shape[1])
        ...
        if time_bins is None:
            time_bins = np.int32([0,1000])  # 1 second if not specified
        self.time_bins = time_bins

        if data is None:
            data = np.zeros((N_axial,N_azimuthal,binning.N_u,binning.N_v))
        elif np.isscalar(data):
            data = data + np.zeros((N_axial,N_azimuthal,binning.N_u,binning.N_v))
    ...
    def get_projection_info(self):
        ...
        return pprint(self.prompts_sparsity_info)
    ...
    def get_subset_pure_python(self, subsets_matrix):
        subsets_matrix = np.int32(subsets_matrix)
    ...
    def crop(self, bins_range):
        ...
        A = bins_range[0]
        B = bins_range[1]
        data = np.ascontiguousarray(self.data[:, :, A:B, :])
        self.binning.size_u = (
    ...
    def apply_noise_Poisson(self):  # TODO add function to apply NB (negative binomial) noise to data
        self.data = np.float32(np.random.poisson(self.data))
    ...
    def display(self, coords=None, clim=None, cmap=None, interp = None, figsize = (20,9), crop=None, autotranspose=True):
        V = TriplanarView(self, crop=crop, autotranspose=autotranspose)
        return V.show(coords = coords, clim=clim, cmap=cmap, figsize = figsize, interp = interp)

    def display_interactive(self, clim=None, cmap=None, figsize = (20,9), crop=None, autotranspose=True):
        V = TriplanarViewInteractive(self, crop=crop, autotranspose=autotranspose)
    ...
    def volume_render(self, azim = None, res = (1.0,1.0,1.0), crop=None, autotranspose=False):
        if azim is None:
            azim = self.binning.N_azimuthal //2
        V = VolumeRenderer(self.data[:,azim,:,:], res=res, crop=crop, autotranspose=autotranspose)
        return V.display()
    ...
    def __truediv__(self, other):
        ...
        if other is None:  # This makes reconstruction code cleaner, when e.g. scatter and randoms are not specified
            return self
        #print(f'same sparsity? {self._is_same_sparsity_(other)}')
        if self._is_same_sparsity_(other):
            other = other.data
        out = self.copy()
        out.data = out.data / other
