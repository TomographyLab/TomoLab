...
# TOMOLAB does not like the concepts of sinogram-span and meshing.
...
class Michelogram:
    def __init__(self, n_rings=64, span=DEFAULT_SPAN, max_ring_difference=60):
        ...
        if not self._verify_max_ring_difference(span, n_rings, max_ring_difference):
            max_ring_difference_actual = self._largest_compatible_max_ring_difference(
                span, n_rings, max_ring_difference
            )
            compatible_max_ring_diff = self._compatible_max_ring_differences(
                span, n_rings
            )
            if compatible_max_ring_diff is []:
                print(
                    "Michelogram: No 'maximum_ring_difference' exists compatible with given 'span' and 'n_rings'. "
                )
            elif max_ring_difference_actual is None:
                print(
                    (
                        "Michelogram: 'maximum_ring_difference' %d too small, using %d. "
                ...
                print(
                    (
                        ...
                        % (max_ring_difference, span)
                        + str(compatible_max_ring_diff)
    ...
    def _make(self):
        ...
        if self.span > 1:
            n_segments_per_side = (
                self.max_ring_difference + 1 - self.span // 2
            ) // self.span
        else:
            n_segments_per_side = self.max_ring_difference
        n_segments = n_segments_per_side * 2 + 1
        segments_sizes = np.zeros([n_segments_per_side + 1], dtype=np.int32)
        if self.span > 1:
        ...
        self.michelogram_sinogram = -1 * np.ones([n_rings, n_rings], order="F", dtype=np.int32)
        self.michelogram_plane = -1 * np.ones([n_rings, n_rings], order="F", dtype=np.int32)
        ...
        if self.span > 1:
            ...
            for X in range(0, self.max_ring_difference, self.span):
                ...
                self._fill_michelogram(
                    X, -n_rings_small, sinogram_index, plane_index_offset
                )
                sinogram_index += 1
    ...
    def _verify_max_ring_difference(self, span, n_rings, max_ring_difference):
        return max_ring_difference in self._compatible_max_ring_differences(
            span, n_rings
        )
    ...
    def _largest_compatible_max_ring_difference(
        self, span, n_rings, max_ring_difference
    ):
