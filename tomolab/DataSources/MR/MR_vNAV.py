...
class vNAV_MPRage:
        ...
        for file_name in files:
            ...
            if file_valid:
                ...
                if from_dicom_comments:
                    ...
                    if N:
                        a = np.float32(s.split(" ")[1:5])
                        t = np.float32(s.split(" ")[6:9])
                        freq = np.float32(s.split(" ")[10])
                        r = angle_axis_to_rotation(a[0], a[1:4])
                    else:
                        t = np.float32([0, 0, 0])
                        r = np.float32([0, 0, 0])
                        a = np.float32([0, 1, 0, 0])  # FIXME: is this right?
                    ...
                    motion_dicom_comments = np.dot(tra_mat, rot_mat)
    ...
    def plot_motion(
        ...
        if display_dicom_comments:
            ...
            # mr = np.min([self._rx_comm,self._ry_comm,self._rz_comm])
            # Mr = np.max([self._rx_comm,self._ry_comm,self._rz_comm])
            # mt = np.min([self._tx_comm,self._ty_comm,self._tz_comm])
            # Mt = np.max([self._tx_comm,self._ty_comm,self._tz_comm])
            mr = range_deg[0]
            Mr = range_deg[1]
            mt = range_mm[0]
            Mt = range_mm[1]
    ...
    def plot_quaternion(
        ...
        s = rad_to_deg(np.asarray(self._a0_comm))[1:]
        ...
        v0 = np.asarray(self._a1_comm[0], self._a2_comm[0], self._a3_comm[0])
        for t in range(self.get_n_time_points()):
            vt = np.asarray(self._a1_comm[t], self._a2_comm[t], self._a3_comm[t])
